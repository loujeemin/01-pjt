...
file_path = Path('./skeleton/data/books_20.json')  # JSON 파일 경로 설정
...
if file_path.exists():  # 파일이 존재할 경우
    ...
    for item in data['item']:  # 'item' 리스트의 각 항목을 순회
        title = item["title"]
        author = item["author"]
        book_authors[title] = author  # 책 제목과 작가 정보를 추출하고 딕셔너리에 추가하는 코드 (book_authors[title] = author)

    # 4. 결과 출력
    # 도서 제목과 작가를 출력합니다.
    print("도서 별 작가 리스트:")
    for title, author in book_authors.items():  # 딕셔너리의 각 항목 순회
        print(f"제목: {title}\n작가: {author}")  # 제목과 작가를 출력하는 코드 (print(f"제목: {title}\n작가: {author}\n"))    
        print('--------------------------------------------')
